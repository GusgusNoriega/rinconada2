...
<body>
        ...
        <x-cabecera-top />
            ...
            <div class="itemInfo active">
                ...
                <div class="cuerpo">
                    <div class="contentInput">
                        <p>CÃƒÂ³digo de socio</p>
                        <input type="text" name="codigo" id="codigo" value="4546" disabled/>
                    </div>
                    <div class="contentInput">
                        <p>Nombres</p>
                    ...
                    <div class="contentInput">
                        ...
                        <select name="nacionalidad" id="nacionalidad" disabled>
                            <option value="PerÃƒÂº">PerÃƒÂº</option>
                    ...
                    <div class="contentInput">
                        <p>NÃ‚Âº de documento de identidad</p>
            ...
            <div class="itemInfo">
                ...
                <div class="cuerpo">
                    ...
                    <div class="contentInput">
                        <p>UbicaciÃƒÂ³n</p>
                        <input type="text" name="ubicacion" id="ubicacion" value="01140140 PerÃƒÂº/Lima/Lima/San Borja" disabled/>
                    </div>
                    <div class="contentInput">
                        <p>TelÃƒÂ©fono fijo</p>
                        <input type="number" name="tel_fijo" id="tel_fijo" value="980500900" disabled/>
                    </div>
                    <div class="contentInput">
                        <p>TelÃƒÂ©fono celular</p>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // FunciÃƒÂ³n para obtener parÃƒÂ¡metros de la URL
            ...
            const userCodigo = getQueryParam('codigo');

            if (!userCodigo ) {
                console.error("No se proporcionÃƒÂ³ un Codigo de usuario en la URL.");
                return;
            }

            // Realizamos la peticiÃƒÂ³n a la API con el ID dinÃƒÂ¡mico
            fetch(`{{ config('app.url') }}/api/users/codigo/${userCodigo}`)
                ...
                .then(json => {
                    if (json.type === 'success') {
                        ...
                        // Asignamos los datos que estÃƒÂ¡n en el nivel principal
                        if (user.name) {
                        ...
                        if (user.foto) {
                            ...
                            if (fotoPerfil) {
                                fotoPerfil.src = `/fotos/${user.foto}`;
                            }
                            if (fotoPerfil2) {
                                fotoPerfil2.src = `/fotos/${user.foto}`;
                            }
                        }

                        if (user.name) {
                            const nombreCom = document.getElementById('nombre-completo');
                            const nombreCod = document.getElementById('nombre-codigo');
                            if (nombreCom) {
                                //cambiar el texto del h3
                                nombreCom.textContent = user.name;

                            }
                            if (nombreCod) {
                                //cambiar el texto del h3
                                nombreCod.innerHTML = user.name + "<br />NÃ‚Âº " + user.detail.codigo;
                            }
                            
                        }

                        // Si la respuesta tiene el objeto 'detail', se asignan los datos
                        if (user.detail) {
                            Object.keys(user.detail).forEach(function(key) {
                ...
                // Selecciona todos los botones de ediciÃƒÂ³n dentro de cada contenedor
                let oldUser = null;
                try {
                oldUser = JSON.parse(localStorage.getItem('datosUserRinconda'));
                } catch (error) {
                console.error('No se pudo parsear localStorage', error);
                }

                // Validamos que existan datos previos
                if(!oldUser) {
                console.warn("No hay datos en localStorage. AsegÃƒÂºrate de que el fetch inicial se haya hecho correctamente.");
                }

                // ------------------------
                // 2) LÃƒâ€œGICA PARA EDITAR/GUARDAR
                // ------------------------
                // Seleccionamos todos los botones "Editar/Guardar" (clases .editar y .editar2)
                const botonesEditar = document.querySelectorAll('.editar, .editar2');

                botonesEditar.forEach(boton => {
                    boton.addEventListener('click', function () {
                        // Ubicamos el contenedor principal (itemInfo)
                        ...
                        // Determinamos si estÃƒÂ¡n deshabilitados
                        const estaDeshabilitado = campos.length > 0 ? campos[0].disabled : false;

                        if (estaDeshabilitado) {
                            // Si estaban deshabilitados, significa que ahora el usuario quiere EDITAR:
                            // Habilitamos todos los campos
                            campos.forEach(campo => campo.disabled = false);

                            // Cambiamos el texto del botÃƒÂ³n a "Guardar"
                            this.innerHTML = `
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none">
                            ...
                            // Si NO estaban deshabilitados, significa que el botÃƒÂ³n se presionÃƒÂ³ para GUARDAR:
                            // 1. Bloqueamos de nuevo
                            campos.forEach(campo => campo.disabled = true);

                            // 2. Cambiamos el texto a "Editar"
                            this.innerHTML = `
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none">
                                <path d="M4.8105 11.9175L12.417 4.31101L11.3565 3.25051L3.75 10.857V11.9175H4.8105ZM5.43225 13.4175H2.25V10.2353L10.8263 1.65901C10.9669 1.51841 11.1576 1.43942 11.3565 1.43942C11.5554 1.43942 11.7461 1.51841 11.8868 1.65901L14.0085 3.78076C14.1491 3.92141 14.2281 4.11214 14.2281 4.31101C14.2281 4.50989 14.1491 4.70062 14.0085 4.84126L5.43225 13.4175ZM2.25 14.9175H15.75V16.4175H2.25V14.9175Z" fill="#003C3E"/>
                            </svg>
                            ...
                            // 3. Ahora COMPARAR y ACTUALIZAR si corresponde
                            if (oldUser) {
                            const huboCambios = prepararYEnviarActualizacion(oldUser);
                            if(!huboCambios) {
                                console.log("No hubo cambios en los campos. No se envÃƒÂ­a actualizaciÃƒÂ³n.");
                            }
                            }
                        }
                    });
                ...
                // ------------------------
                // 3) FUNCIÃƒâ€œN PARA PREPARAR Y ENVIAR LA ACTUALIZACIÃƒâ€œN
                // ------------------------
                function prepararYEnviarActualizacion(oldUser) {
                    // Obtenemos todos los campos a nivel global que necesitamos (o puedes buscar por contenedor, segÃƒÂºn tu preferencia)
                    
                    // Campos top-level (por ejemplo, "email")
                    ...
                    const newTipoDoc = document.getElementById('tipo_doc')?.value || '';
                    const newNroDoc = document.getElementById('nro_doc')?.value.trim() || '';
                    ...
                    // Centro de estudio, profesiÃƒÂ³n, etc.
                    ...
                    const newTelefonoEmpresa = document.getElementById('telefono_empresa')?.value.trim() || '';
                    ...
                    // Armamos el objeto nuevo (el payload que requiere tu endpoint)
                    const updatedData = {
                    email: newEmail,
                    detail: {
                        nombres: newNombres,
                        ...
                        tel_fijo: newTelFijo,
                        tel_cel: newTelCel,
                        email: newEmail, // Ojo: a veces se repite en 'detail' segÃƒÂºn tu endpoint
                        centro_estudio: newCentroEstudio,
                        especialidad: newEspecialidad,
                        profesion: newProfesion,
                        telefono_empresa: newTelefonoEmpresa,
                    ...
                    // Ahora comparamos con oldUser para ver si hubo algÃƒÂºn cambio
                    let hayDiferencias = false;

                    // Comparamos top-level
                    ...
                    // Comparamos detail
                    if (oldUser.detail) {
                    // Recorremos cada campo que enviamos en 'detail':
                    ...
                    // Si no hay detail en oldUser, entonces definitivamente es un cambio
                    hayDiferencias = true;
                    }
                    ...
                    // ------------------------
                    // 4) REALIZAR EL FETCH DE ACTUALIZACIÃƒâ€œN
                    // ------------------------
                    // Tomar el "id" para la ruta /api/users/{id}/limited
                    const userId = oldUser.id; // AsegÃƒÂºrate de que en el localStorage guardaste .id

                    fetch(`{{ config('app.url') }}/api/users/${userId}/limited`, {
                    method: 'PUT', // o 'PATCH' segÃƒÂºn definas en tu API
                    headers: {
                    ...
                    if (data.type === 'success') {
                        // 5) Volvemos a recargar la pÃƒÂ¡gina (o puedes volver a pedir getUser si lo deseas)
                        location.reload();
                    } else {
                        alert('Hubo un error al actualizar: ' + (data.message || 'Error desconocido'));
                    }
                    })
                    .catch(err => {
                    console.error(err);
                    alert('Hubo un error en la peticiÃƒÂ³n: ' + err);
                    });

                    return true; // Indica que sÃƒÂ­ hubo cambios y se enviÃƒÂ³ la peticiÃƒÂ³n
                }
